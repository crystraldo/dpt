...
def main():
    ...
    if CONFIG.phase.lower() == "train":
        ...
        train_dataset = DataGenerator(data_path=CONFIG.data.train1k_path,phase='train')
        test_image_file = ImageFileTest(alpha_dir=CONFIG.data.test_alpha, merged_dir=CONFIG.data.test_merged, trimap_dir=CONFIG.data.test_trimap)
        ...
        test_dataloader = DataLoader(test_dataset,batch_size=1,shuffle=False,num_workers=CONFIG.data.workers,sampler=test_sampler,drop_last=False)
        """=== Set Trainer ==="""
...
if __name__ == '__main__':
    ...
    # Composition-1k
    parser.add_argument('--config', type=str, default='config/dpt.toml')
