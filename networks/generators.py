...
from networks.encoders.DPT import DPT


def get_generator(is_train=True):
    generator = Generator_DPT(is_train=is_train)
    return generator


class Generator_DPT(nn.Module):

    def __init__(self, is_train=True):

        super(Generator_DPT, self).__init__()
        self.encoder = DPT(embed_dim=96,
