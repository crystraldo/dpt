...
class ResNet_Seg_Dec(nn.Module):
    ...
    def forward(self, x, mid_fea):
        ...
        x = self.conv1(x)
        x = self.bn1(x)
        keep_emb = self.leaky_relu(x)
       
        out = self.out_conv(keep_emb) 
        return out, keep_emb

class ResShortCut_Seg_Decoder(ResNet_Seg_Dec):

    def __init__(self, block, layers, norm_layer=None, large_kernel=False, late_downsample=False):
        super(ResShortCut_Seg_Decoder, self).__init__(block, layers, norm_layer, large_kernel, late_downsample=late_downsample)
    ...
    def forward(self, x, mid_fea:List[torch.Tensor]):
        ...
        x = self.conv1(x)
        x = self.bn1(x)
        keep_emb = self.leaky_relu(x) + fea1
